...
class Que_ans extends MY_Controller {

	public function __construct()
	{
		parent::__construct();
		$this->redirect = admin("que_ans");
	}

	private $name = 'que_ans';
	...
	public function get()
    {
        $fetch_data = $this->main->make_datatables('Que_ans_model');
        $sr = $_POST['start'] + 1;
        $data = array();
	...
	public function update($id)
	{
        ...
        if ($this->form_validation->run() == FALSE)
        {
			...
			$data['data'] = $this->main->get($this->table, 'id, questions, answers', ['id' => d_id($id)]);
